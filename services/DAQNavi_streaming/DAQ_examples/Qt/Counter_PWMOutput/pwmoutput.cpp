...
void PWMOutput::ConfigureDevice()
{
    ...
    errorCode = pwModulatorCtrl->setSelectedDevice(selected);
    CheckError(errorCode);

    std::wstring profile = configure.profilePath.toStdWString();
    errorCode = pwModulatorCtrl->LoadProfile(profile.c_str());
    CheckError(errorCode);
...
void PWMOutput::ButtonStartClicked()
{
     ...
     for (int i = pwModulatorCtrl->getChannelStart(); i < pwModulatorCtrl->getChannelStart() +
          pwModulatorCtrl->getChannelCount(); i++)
     {
