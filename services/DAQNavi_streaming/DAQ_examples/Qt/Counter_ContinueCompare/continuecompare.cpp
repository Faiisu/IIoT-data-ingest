...
{
    //Logout the edge
    ErrorCode errorCode = udCounterCtrl->Logout();
    ...
    errorCode = udCounterCtrl->setSelectedDevice(selected);
    CheckError(errorCode);
    ...
    for(int i = udCounterCtrl->getChannelStart(); i < udCounterCtrl->getChannelStart() +
        udCounterCtrl->getChannelCount(); i++)
    {
...
void ContinueCompare::TimerTicked()
{
    ...
    CheckError(errorCode);
    udCounterCtrl->Read(1, (int32*)&m_counterValue);
...
void ContinueCompare::ButtonStartClicked()
{
    ...
    if (dispersed == m_contCompTabSel) {
        ...
        udCounterCtrl->CompareAppendTable(udCounterCtrl->getChannelStart(), 3, (int32*)ComTable);
    } else {
        CreateLineTable(0, param);
        udCounterCtrl->CompareAppendInterval(udCounterCtrl->getChannelStart(), param.firstValue,
                                             param.increment, param.count);
    }
    m_endCount = 0;
    m_matchCount = 0;
    m_tabIndex = 0;
...
void ContinueCompare::ButtonStopClicked()
{
    ...
    ErrorCode errorCode = udCounterCtrl->CompareClear();
    CheckError(errorCode);
    errorCode = udCounterCtrl->setEnabled(false);
    CheckError(errorCode);
}
...
void ContinueCompare::OnUdCounterEvent(void * sender, UdCntrEventArgs * args, void * userParam)
{
   ...
   if (EvtCntCompareTableEnd0 + channelStart <= args->Id && args->Id <EvtCntCompareTableEnd0 +
           channelStart + channelCount)
   {
      ...
      if (uParam->m_contCompTabSel == dispersed)
      {
         ...
         udCounterCtrl->CompareAppendTable(channelStart, 3, (int32*)comTable);
      }else{
         uParam->CreateLineTable(uParam->m_tabIndex, lineTabparam);
         udCounterCtrl->CompareAppendInterval(channelStart, lineTabparam.firstValue, lineTabparam.increment,
                                              lineTabparam.count);
      }
      matched = true;
   } else if (EvtCntPatternMatch0 + channelStart <= args->Id &&
              args->Id <= EvtCntPatternMatch0 + channelStart + channelCount)
   {
      uParam->m_matchCount++;
      matched = true;
   }
