...
void SimpleGraph::paintEvent(QPaintEvent *)
{
    QPainter p(this);
    ...
    for (int index = 0; index < m_copyDataCountPerChan; ++index)
    {
        ...
        for (int i = 0; i < m_plotCount; i++)
        {
            ...
            m_dataPointBuffer[dataIndex].setY((long)ceil(YCordDividedRate * 
                (m_yCordRangeMax - m_drawDataBuffer[(int)(m_plotCount * (index + m_mapDataIndexPerPlot +
                                                                         m_shiftCount) + i)])));
                                ...
                                TimeUnit unit)
{
    ...
    rangeStrs[0] = QString::number(rangeMax, 'g');
    rangeStrs[0] += spted_unit[tm_unit_idx];
    rangeStrs[1] = QString::number(rangeMin, 'g');
