    ...
    : QDialog(parent, flags)
{
    ...
    connect(this->buttonGroup1, SIGNAL(buttonPressed(QAbstractButton *)), this, SLOT(WaveButtonClicked(QAbstractButton *)));
    connect(this->buttonGroup2, SIGNAL(buttonPressed(QAbstractButton *)), this, SLOT(ManualClicked(QAbstractButton *)));
}
...
void StaticAO::ManualClicked(QAbstractButton *btn)
{
    int ch, baseIdx;
    int id = buttonGroup2->id(btn);
...
void StaticAO::TimerTicked()
{
    for (int i = 0; i < 2; i++) {
        if (m_waveSeled[i]) {
            dataScaled[i] = waveformGenerator->GetOnePoint(m_waveParam[i].Style, m_wavePtIdx[i],
                                                           m_waveParam[i].HighLevel, m_waveParam[i].LowLevel);
