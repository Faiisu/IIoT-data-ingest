...
StreamingAI_TDtr::StreamingAI_TDtr(QWidget *parent, Qt::WindowFlags flags)
    : QDialog(parent, flags), is_pause(false)
{
    ui.setupUi(this);
...
void StreamingAI_TDtr::ConfigureDevice()
{
    ...
    CheckError(errorCode);
    errorCode = waveformAiCtrl->getRecord()->setSectionCount(sectionCount);//The 0 means 'streaming' mode.
    CheckError(errorCode);
...
// 2.In theory, the amount of data argsCount returned by a DataReady should be equal to
// sectionLen * channelCount.
// However, in reality, when the dataReady callback function is executed is also affected by system load,
// so it cannot be guaranteed that the amount of data argsCount in each DataReady is equal to
// sectionLen * channelCount.
void StreamingAI_TDtr::OnDataReadyEvent(void * sender, BfdAiEventArgs * args, void * userParam)
{
    ...
    do
    {
        ...
        ErrorCode ret = ((WaveformAiCtrl*)sender)->GetData(getDataCount, uParam->scaledData, 0,
                                                           &returnedCount, NULL, NULL, NULL);
        ...
        uParam->graph->Chart(uParam->scaledData, uParam->configure.channelCount,
                             returnedCount / uParam->configure.channelCount, uParam->xInc);
...
{
    // Please consider if it's necessary to stop the application when this event is triggered.
    /*
