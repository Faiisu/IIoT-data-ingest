...
void DIInterrupt::ConfigureDevice()
{
    ...
    //Logout the edge
    errorCode = instantDiCtrl->Logout();
    ...
    QStringList strs;
    strs << tr(" INT channel") <<
            tr("DI ports value (Port 0~") + QString("%1").arg(instantDiCtrl->getPortCount() - 1) + tr(")");
        ...
        for (int j = 0; j < configureDialog->enablePortCount; j++)
            for (int k = 0; k < 8; k++) {
                if (((configure.enableChannel[j] >> k) & 0x1) == 1 &&
                        interruptChans->getItem(i).getChannel() == configure.selectedPortIndex[j] * 8 + k)
                {
...
void DIInterrupt::OnDiSnapEvent(void *, DiSnapEventArgs * args, void *userParam)
{
