...
void FrequencyMeasurement::ConfigureDevice()
{
    ...
    //Logout the edge
    ErrorCode errorCode = freqMeterCtrl->Logout();
    ...
    errorCode = freqMeterCtrl->setSelectedDevice(selected);
    CheckError(errorCode);

    std::wstring profile = configure.profilePath.toStdWString();
    errorCode = freqMeterCtrl->LoadProfile(profile.c_str());
    CheckError(errorCode);
...
void FrequencyMeasurement::timerTrackBarChanged(int)
{
