...
# Resolve config.json path from the daq_navi directory
CONFIG_PATH = os.path.join(os.path.dirname(__file__), '..', 'daq_navi', 'config.json')
MODE_PATH = os.path.join(os.path.dirname(__file__), '..', 'daq_navi', '.daq_process.mode')

def get_db_dsn():
    """
    Reads the default database DSN from environment variable DB_DSN or config.json.
    ...
    try:
        ...
        return "postgresql://admin:admin@localhost:5432/daq_db"
