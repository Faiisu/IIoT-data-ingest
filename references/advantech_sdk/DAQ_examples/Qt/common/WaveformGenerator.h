...
public:
      ...
      case triangle:
         {
            ...
            {
               data = amplitude * ((2.0 * (m_pointPerPeriod / 4.0) - index) / (m_pointPerPeriod / 4.0)) +
                       offset;
            }
            else
            {
   ...
   void GetMultipleWaveform(double *waveform, int waveCount, WaveformParameter *waveParam)
   {
      for (int i = 0; i < m_pointPerPeriod;i++ )
      {
         ...
         {
            waveform[j + i * waveCount] = GetOnePoint(waveParam[j].Style, i, waveParam[j].HighLevel,
                                                      waveParam[j].LowLevel);
         }
      }
   }
};
