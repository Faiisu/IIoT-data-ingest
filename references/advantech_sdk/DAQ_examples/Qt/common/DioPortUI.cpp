    ...
    : QWidget(parent, flags)
{
    ...
    connect(buttonGroup, SIGNAL(buttonPressed(QAbstractButton *)), this, SLOT(ButtonClicked(QAbstractButton *)));
}
...
void DioPortUI::ButtonClicked(QAbstractButton *btn) {
    int id = buttonGroup->id(btn);
    if (this->isEditable) {
        if ((mask >> (7 - id) & 0x1) == 1) {
            if ((state >> (7 - id) & 0x1) == 1) {
