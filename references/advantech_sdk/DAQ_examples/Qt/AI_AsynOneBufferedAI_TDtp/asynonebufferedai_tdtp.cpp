...
{
    setWindowTitle(tr("Asynchronous One Buffered AI with Trigger Delay to Stop - Run(") +
                   configure.deviceName + tr(")"));
...
void AsynOneBufferedAI_TDtp::ConfigureDevice()
{
    ...
    ///Logout the edge
    errorCode = waveformAiCtrl->Logout();
    ...
    CheckError(errorCode);
    errorCode = waveformAiCtrl->getRecord()->setSectionCount(sectionCount);//The 0 means "One Buffered" mode.
    CheckError(errorCode);
    ...
    if (configure.triggerIndex == 0)
    {
       if (waveformAiCtrl->getFeatures()->getTriggerCount() > 0)
       {
         ...
         if (waveformAiCtrl->getFeatures()->getTriggerSupported()) {
   ...
   }else if (configure.triggerIndex == 1)
   {
      ...
      if (waveformAiCtrl->getFeatures()->getTriggerCount() > 1)
      {
         ...
         if (waveformAiCtrl->getFeatures()->getTrigger1Supported()) {
...
void AsynOneBufferedAI_TDtp::OnStoppedEvent(void * sender, BfdAiEventArgs * args, void * userParam)
{
   ...
   int32 chanCount = uParam->waveformAiCtrl->getConversion()->getChannelCount();
   int32 bufSize = (uParam->configure.sectionLength * uParam->configure.channelCount);

   do
   {
       getDataCount = qMin(bufSize, remainingCount);
       ((WaveformAiCtrl *)sender)->GetData(args->Count, uParam->scaledData, 0, &returnedCount, NULL, NULL,
                                           NULL);
       remainingCount -= returnedCount;
       uParam->dataReadyCount = returnedCount;
...
void AsynOneBufferedAI_TDtp::GraphInitialize(int samplesCountPerChan) {
    ...
    graph->Clear();
    graph->Chart(scaledData, configure.channelCount, dataReadyCount /configure.channelCount,
                 1.0 / waveformAiCtrl->getConversion()->getClockRate());
...
void AsynOneBufferedAI_TDtp::TriggerFlag()
{
    if (isTriggerSupported && !isSourceNone) {
        ...
        int relativeWidth = GetArrowXCordRelativeWidth(
                    dataReadyCount / waveformAiCtrl->getConversion()->getChannelCount(),
                    waveformAiCtrl->getTrigger()->getDelayCount(), ui.graphFrame->width());
        ...
        if (relativeWidth == 0) {
            ...
            ui.triggerPointFlag->setVisible(true);
            ui.label_triggerPoint->move(ui.label_triggerPoint->x() + relativeWidth,
                                        ui.label_triggerPoint->y());
            ui.label_triggerPoint->setVisible(true);
        }
    }
}
