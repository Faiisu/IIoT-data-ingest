    ...
    : QDialog(parent)
{
    ...
    ui.txtClockRate->setValidator(new QIntValidator(0, 30000000, this));
...
void ConfigureDialog::Initailization()
{
    ...
    if (configure.hostName != LOCAL_EDGE) {
        ...
        if (Success == bfdAoCtrl->Login(hostName.c_str())) {
            EnableSettings(true);
        }
        else {
    ...
    Array<DeviceTreeNode> *supportedDevices = bfdAoCtrl->getSupportedDevices();
...
void ConfigureDialog::DeviceChanged(int)
{
    ...
    ErrorCode errorCode = bfdAoCtrl->setSelectedDevice(selected);
    ...
    int channelCount = (bfdAoCtrl->getChannelCount() < 2) ?
        bfdAoCtrl->getChannelCount() : 2;
    ...
    for (int i = 0; i < ValueRanges->getCount(); i++)
    {
        if (ValueRanges->getItem(i) < UserCustomizedVrgStart) {
            errorCode = AdxGetValueRangeInformation(ValueRanges->getItem(i), sizeof(vrgDescription),
                                                    vrgDescription, &ranges, NULL);
...
void ConfigureDialog::ButtonOKClicked()
{
    ...
    configure.clockRate = ui.txtClockRate->text().toDouble();
...
void ConfigureDialog::EnableSettings(bool is_enable)
{
    ...
    ui.groupBox->setEnabled(is_enable);
    ui.groupBox_2->setEnabled(is_enable);
}
