...
void DelayedPulseGeneration::ConfigureDevice()
{
    ...
    errorCode = oneShotCtrl->setSelectedDevice(selected);
    CheckError(errorCode);

    std::wstring profile = configure.profilePath.toStdWString();
    errorCode = oneShotCtrl->LoadProfile(profile.c_str());
    CheckError(errorCode);
...
void DelayedPulseGeneration::OnCounterEvent(void *, CntrEventArgs *, void * userParam)
{
...
void DelayedPulseGeneration::ButtonStartClicked()
{
    ...
    for (int32 i = oneShotCtrl->getChannelStart();
         i < oneShotCtrl->getChannelStart() + oneShotCtrl->getChannelCount(); i ++)
    {
