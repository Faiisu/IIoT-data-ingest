    ...
    : QDialog(parent, flags), m_thread(this)
{
    ...
    connect(this,                SIGNAL(update_state_signal(int, CaliState)),  this,
            SLOT(on_update_state(int, CaliState)));
    connect(this,                SIGNAL(update_val_signal(int)),               this,
            SLOT(on_update_value(int)));
}
...
void CalibrationItem::Initialize() {
    ...
    qDebug() << "Device Name[" << configure.deviceName << "], Solution Name[" << configure.slnName <<
                "], Solution Index[" << configure.slnIdx << "].";
...
void CalibrationItem::ConfigureDevice() {
    ...
    errorCode = m_caliCtrl->setSelectedDevice(selected);
    CheckError(errorCode);
    ...
    if(m_matching_ai_manual || m_matching_ao_manual){
        ...
        CaliState caliState = m_subj_list->getItem(m_subj_idx).AdjCodeGetCurrentState(&value,
                                                                                      &codeCount,
                                                                                      adjustCode);
        qDebug() << "---ConfigureDevice---Cali State[" << caliState << "], Value:" << value <<
                    ", Codde Count:" << codeCount << ", Adjust Code[" << codeCount - 1 << "] = " <<
                    adjustCode[codeCount - 1];
...
{        
    ...
    if(m_matching_ai_manual || m_matching_ao_manual){
        ...
        int32  codeCount = 0;
        int32  adjustCode[RELATED_CNT_MAX] = {0};
        CaliState caliState = m_subj_list->getItem(m_subj_idx).AdjCodeGetCurrentState(&value, &codeCount,
                                                                                      adjustCode);
        qDebug() << "---onCmbSubj---Cali State[" << caliState << "], Value:" << value <<
                    ", Codde Count:" << codeCount << ", Adjust Code[" << codeCount - 1 << "] = " <<
                    adjustCode[codeCount - 1];
...
{    
    ...
    m_subj_list->getItem(m_subj_idx).ManualAdjSetCode(1, &cali_data);
...
void CalibrationItem::TimerTicked1()
{
    ...
    int32   idx = codeCount - 1;
    ui.lineEdit_read_value->setText(QString::number(value));
    qDebug() << "---TimerTicked-1---Cali State[" << caliState << "], Value:" << value <<
                ", Codde Count:" << codeCount << ", Adjust Code[" << idx << "] = " << adjustCode[idx];
...
void CalibrationItem::CheckCurrentState()
{
    ...
    do{
        if(m_matching_ai_auto || m_matching_ao_auto){
            ...
            {
                caliState = m_subj_list->getItem(i).AdjCodeGetCurrentState(&value, &codeCount, adjustCode);
                qDebug() << "--TimerTicked--Row[" << i << "], Cali State[" << caliState << "], Value:" <<
                            value << ", Codde Count:" << codeCount << ", Adjust Code = " << adjustCode[0];
                QThread::msleep(10);
        ...
        if(m_matching_ai_manual || m_matching_ao_manual){
            ...
            caliState = m_subj_list->getItem(m_subj_idx).AdjCodeGetCurrentState(&value, &codeCount,
                                                                                adjustCode);
            QThread::msleep(500);
            //int32   idx = codeCount - 1;
...
void CalibrationItem::SectionTypeCheck()
{
    ...
    m_matching_ai_auto = !(QString::compare(QString::fromWCharArray(m_strPtr),
                                            "Cali.Section.InSignals.Batch"));
    m_matching_ai_manual |= !(QString::compare(QString::fromWCharArray(m_strPtr),
                                               "Cali.Section.InSignals.DigitalVR"));

    m_matching_ao_auto = !(QString::compare(QString::fromWCharArray(m_strPtr),
                                            "Cali.Section.OutSignals.Batch"));
    m_matching_ao_manual |= !(QString::compare(QString::fromWCharArray(m_strPtr),
                                               "Cali.Section.OutSignals.DigitalVR"));
}
...
void CalibrationItem::on_update_state(int idx, CaliState state)
{
    ...
    if(!m_subjCaliDone[idx]){
        ...
        if(caliState == CaliFailed || (idx == lastOneIdx) && (caliState == CaliSuccess ||
                                                              caliState == CaliFailed)){
            ui.btn_save->setEnabled(true);
            ButtonStopClicked();
            m_cali_stop = true;
            m_thread.terminate();
