...
void PulseWidthMeasurement::ConfigureDevice()
{
    ...
    errorCode = pwMeterCtrl->setSelectedDevice(selected);
    CheckError(errorCode);

    std::wstring profile = configure.profilePath.toStdWString();
    errorCode = pwMeterCtrl->LoadProfile(profile.c_str());
    CheckError(errorCode);
