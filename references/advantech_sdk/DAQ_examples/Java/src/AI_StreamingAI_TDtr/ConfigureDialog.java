...
public class ConfigureDialog extends JDialog {
	...
	public ConfigureDialog(StreamingAI_TDtr parrent) {
		...
		panel.setBounds(283, 66, 311, 159);
		...
		panel_1.setBounds(283, 234, 311, 159);
	...
	public void LoadConfiguration(ConfigureParameter config){
		...
		cmbHostName.setSelectedItem(config.hostName);
	}
	...
	class ButtonOKActionListener implements ActionListener{
		public void actionPerformed(ActionEvent arg0) {
			...
			if(isTriggerSupported){
				...
				boolean init = wfAiCtrl.getInitialized();
				
				DeviceInformation info = wfAiCtrl.getSelectedDevice();
	...
	protected void SetEditUIEnable(boolean enable) {
		...
		cmbChannelStart.setEnabled(enable);
		cmbChannelCount.setEnabled(enable);
		cmbValueRange.setEnabled(enable);
		btnOK.setEnabled(enable);
		btnBrowse.setEnabled(enable);
		txtProfilePath.setEnabled(enable);
		txtSectionLength.setEnabled(enable);
		txtClockRatePerChan.setEnabled(enable);
		cmbTriggerSource.setEnabled(enable);
		cmbTriggerEdge.setEnabled(enable);
		txtTriggerLevel.setEnabled(enable);
		txtDelayCount.setEnabled(enable);		
		cmbTrigger1Source.setEnabled(enable);
		cmbTrigger1Edge.setEnabled(enable);
		txtTrigger1Level.setEnabled(enable);
		txtDelayCount1.setEnabled(enable);
	}
	...
	class ButtonLoginActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!hostName.equals(localEdge)) {
				if (ErrorCode.Success == wfAiCtrl.Login(hostName)) {
