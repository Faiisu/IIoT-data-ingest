...
public class StaticDO extends JFrame {
	...
	public void Initialization() {
		String deviceName = configure.deviceName;
		setTitle("Static DO - Run(" + deviceName.substring(0, deviceName.length() - 1) + ")");
		
		try {
			ErrorCode ret = instantDoCtrl.Logout();
			if (ret != ErrorCode.Success) {
				throw new Exception(String.format("Logout edge failed, %s", ret.toString()));
			}
			String edgeName = configure.hostName;
			if (!edgeName.equals(localEdge)) {
				ret = instantDoCtrl.Login(edgeName);
				if (ret != ErrorCode.Success) {
					throw new Exception(String.format("Login edge %s failed, %s", edgeName, ret.toString()));
				}
			}
			
			instantDoCtrl.setSelectedDevice(new DeviceInformation(deviceName));
		} catch (Exception ex) {
